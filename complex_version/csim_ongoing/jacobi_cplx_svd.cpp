...
void normalize_matrix(Complex *A, int rows, int cols, T &max_val) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            T norm = complex_norm(A[i * cols + j]);
            if (norm > max_val) {
                max_val = norm;
            }
        }
...
void restore_sigma(Complex *SIGMA, int rows, int cols, T max_val) {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            SIGMA[i * cols + j] = SIGMA[i * cols + j] * max_val;
        }
    }
}
...
void jacobi_svd(Complex* A, Complex* u, Complex* v, Complex* s){
    ...
    std::cout << "------A before normalize-----"<< std::endl;
    print_mxn_matrix(A, m, n);

    normalize_matrix(A, m, n, max_val);
    ...
    for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                uSv_H[i * n + j] = uSv_H[i * n + j] * max_val;
            }
        }

        std::cout << "------denormalized usv_H-----" << std::endl;
